...
public class TowerTracker {
	...
	public static final Scalar RED = new Scalar(0, 0, 255), BLUE = new Scalar(255, 0, 0), GREEN = new Scalar(0, 255, 0),
			BLACK = new Scalar(255, 255, 255), YELLOW = new Scalar(0, 255, 255),
			// these are the threshold values in order
			LOWER_BOUNDS = new Scalar(200, 200, 0), UPPER_BOUNDS = new Scalar(255, 255, 15);
	...
	public static final int CAP_PROP_FRAME_WIDTH = 3, CAP_PROP_FRAME_HEIGHT = 4, CAP_PROP_BRIGHTNESS = 10,
			CAP_PROP_CONTRST = 11, CAP_PROP_SATURATION = 12, CAP_PROP_HUE = 13, CAP_PROP_GAIN = 14,
			CAP_PROP_EXPOSURE = 15, CAP_PROP_WHITE_BALANCE_BLUE_U = 17, CAP_PROP_GAMMA = 22,
	...
	public static void main(String[] args) {
		...
		/**
		 * 
		 * reads an image from a live image capture and outputs information to
		 * the SmartDashboard or a file
		 */
		...
		while (true) {
			...
			try {
				if (time - lastTime >= -1) {
					...
					// Core.inRange(matOriginal, LOWER_BOUNDS, UPPER_BOUNDS,
					// matThresh);
					...
					for (Iterator<MatOfPoint> iterator = contours.iterator(); iterator.hasNext();) {
						...
						Imgproc.approxPolyDP(matOfPoint2f, approxPoly2f, Imgproc.arcLength(matOfPoint2f, true) * .02,
								true);
					...
					// if there is only 1 target, then we have found the target
					// we want
					...
					if (contours.size() >= 1) {
						...
						for (int i = 0; i < contours.size(); i++) {
							...
							// "fun" math brought to you by miss daisy (team
							// 341)!
							...
							Imgproc.putText(matOriginal, "" + (int) distance, center, Core.FONT_HERSHEY_PLAIN, 1,
									BLACK);
							Imgproc.putText(matOriginal, "" + (int) azimuth, centerw, Core.FONT_HERSHEY_PLAIN, 1,
									BLACK);
